...
class PrivacyPolicy:
    ...
    def __post_init__(self):
        if self.pii_fields is None:
            self.pii_fields = {
                "driver_id", "driver_name", "phone", "email",
                "license_plate", "vin", "passenger_faces",
            }
...
class PrivacyEngine:
    ...
    def __init__(self, policy: Optional[PrivacyPolicy] = None):
        ...
        self._pii_patterns = {
            ...
            "license_plate": re.compile(r'[A-Z0-9]{2,3}[-\s]?[A-Z0-9]{2,4}[-\s]?[A-Z0-9]{2,4}'),
        ...
        if level == AnonymizationLevel.FULL:
            ...
            if "location" in result and isinstance(result["location"], dict):
                if "latitude" in result["location"]:
                    result["location"]["latitude"] = round(result["location"]["latitude"], 2)
                if "longitude" in result["location"]:
                    result["location"]["longitude"] = round(result["location"]["longitude"], 2)
...
if __name__ == "__main__":
    ...
    for entry in engine.get_audit_log(limit=5):
        print(f"  [{entry.timestamp.strftime('%H:%M:%S')}] {entry.operation}: {entry.reason}")
